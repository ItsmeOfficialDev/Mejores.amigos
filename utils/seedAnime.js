...
const ApprovedAnime = new mongoose.Schema({
    title: { type: String, required: true, unique: true },
    titleLower: { type: String, required: true, unique: true }
});

const Model = mongoose.model('ApprovedAnime', ApprovedAnime);
...
const animeList = [
    ...
    "Grave of the Fireflies", "Grimgar ashes and illusions", "Guilty Crown", "Hajime No Ippo", "Haikyu!!",
...
async function seed() {
    try {
        ...
        const docs = animeList.map(t => ({ title: t, titleLower: t.toLowerCase() }));
        await Model.insertMany(docs);
        console.log(`Successfully seeded ${docs.length} anime titles.`);
        process.exit(0);
    } catch (err) {
